...
namespace X2295
{
	...
	auto D3DOverlay::CreateImGuiContext() -> VOID
	{
		...
		//vStyle->Colors[ImGuiCol_ChildWindowBg] = ImVec4(0.07f, 0.07f, 0.09f, 1.00f);
	...
	auto D3DOverlay::GetOverlayWindow() -> HWND
	{
		return m_vWnd;
	}
	...
	auto D3DOverlay::InitOverlay(LPCTSTR aWndClassName, LPCTSTR aWndName) -> BOOLEAN
	{
		...
		{
			CleanupOverlay();
			printf(("Cannot create overlay window!"));
			return FALSE;
		}
		if (!CreateDeviceD3D())
		{
			CleanupOverlay();
			printf(("Cannot create d3d device!"));
			return FALSE;
		}
		CreateImGuiContext();
		return true;
